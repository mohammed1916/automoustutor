...
interface DashboardProps {
  ...
  curriculum: { id: string; title: string; description: string; topics: string[] }[];
}

const Dashboard: React.FC<DashboardProps> = ({ learnerState, lastAgentResponse, curriculum }) => {
  const chartData = curriculum.map(week => ({
    subject: week.id,
    A: learnerState.masteryLevels[week.id] ?? 0,
    fullMark: 100,
  }));
    ...
    <div className="h-full bg-slate-900 border-l border-slate-700 flex flex-col overflow-y-auto text-slate-200">
      ...
      <div className="grid grid-cols-2 gap-4 p-4">
        <div className="bg-slate-800 p-3 rounded-lg border border-slate-700">
          ...
          <div className="font-semibold text-sm truncate" title={learnerState.focusTopic}>
            {learnerState.focusTopic}
          </div>
          <div className="text-xs text-cyan-500 mt-1">{learnerState.currentWeek}</div>
        </div>
